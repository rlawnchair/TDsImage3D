...
public class Contour_Analysis implements PlugInFilter {
	...
	public void run( ImageProcessor ip ) {
		...
		//Affichage des points dominants correspondant aux maxima
		ipzc.setColor(Color.red);
		for(int i = 0; i < p.getMaxs().size();i++){
			ipzc.drawOval(getIVisu(p.getContourX(p.getMaxs().get(i))), getJVisu(p.getContourY(p.getMaxs().get(i))), 10, 10);
		}
		//Affichage des points dominants correspondant aux minima
		ipzc.setColor(Color.blue);
		for(int i = 0; i < p.getMins().size();i++){
			ipzc.drawOval(getIVisu(p.getContourX(p.getMins().get(i))), getJVisu(p.getContourY(p.getMins().get(i))), 10, 10);
		}
		ImagePlus visu = new ImagePlus("Discrete contour + normals + points dominants", ipzc);
