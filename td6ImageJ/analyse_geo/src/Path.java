...
public class Path {
	...
	private ArrayList<Integer> mins = new ArrayList<Integer>(); //Stockage des minima
	private ArrayList<Integer> maxs = new ArrayList<Integer>(); //Stockage des maxima
	double threshold = 0.2; //a modifier selon l'image 0.2 fonctionne bien avec les 2 étoiles
	...
	Path(FreemanCode c) {
		...
		computeCurvature(3);
		//computePerimeter(c);
		findExtrema(threshold);
		System.out.println("Maxima :");
		for(int i = 0; i < maxs.size();i++) {
			System.out.println("Indice : "+maxs.get(i)+" Point de courbure : "+_tabPts[maxs.get(i)]._curvature);
			System.out.println("Coordonnées : "+_tabPts[maxs.get(i)]._x+" , "+_tabPts[maxs.get(i)]._y);
		}
		System.out.println("Minima :");
		for(int i = 0; i < mins.size();i++) {
			System.out.println("Indice : "+mins.get(i)+" Point de courbure : "+_tabPts[mins.get(i)]._curvature);
			System.out.println("Coordonnées : "+_tabPts[mins.get(i)]._x+" , "+_tabPts[mins.get(i)]._y);
		}
	}
